...
const int CNFCProtocol::Analyze(IN const CString& strRevData)
{
	...
	try
	{
		...
		sprintf(chs_debug,"abc,%d",Path);
		theApp.RecordLog(chs_debug);
		*/
		//分析数据，路由分大类
		switch(Path)
		{
		...
		case 0x04:
			{
				if( 0x52==BinData[0] )
				{//软件版本号 
					std::vector<std::vector<unsigned char>> dev_info = SplitCharArray(byRevData,iBinLen,0x1C);
					...
					if(!dev_info[1].empty())
					{
						...
						std::copy(dev_info[1].begin(),dev_info[1].end(),GetGlobalApp()->m_NfcPara.SoftVersion);
					}
					

				}
				if( (0x35==BinData[0]) &&(0x37==BinData[1]))
				{//信息查询
					...
					if(!dev_info[1].empty())
					{
						std::copy(dev_info[1].begin(),dev_info[1].end(),GetGlobalApp()->m_NfcPara.Company);
					}
					if(!dev_info[2].empty())
					{
						std::copy(dev_info[2].begin(),dev_info[2].end(),GetGlobalApp()->m_NfcPara.Type);
					}
					if(!dev_info[3].empty())
					{
						std::copy(dev_info[3].begin(),dev_info[3].end(),GetGlobalApp()->m_NfcPara.SerialNumber);
					}
					GetGlobalApp()->m_NfcPara.valid = true;
					char chs[1000]={0};
					//sprintf(chs,"company:%s,type:%s,serialno:%s",GetGlobalApp()->m_NfcPara.Company,GetGlobalApp()->m_NfcPara.Type,GetGlobalApp()->m_NfcPara.SerialNumber);
					...
					)
				{
					...
					//add 2018-06
					memset(GetGlobalApp()->m_NfcPara.TerminalNo,0,sizeof(GetGlobalApp()->m_NfcPara.TerminalNo));
					memset(GetGlobalApp()->m_NfcPara.MerchantCode,0,sizeof(GetGlobalApp()->m_NfcPara.MerchantCode));
					memcpy(GetGlobalApp()->m_NfcPara.TerminalNo,TerminalBin,8);
					memcpy(GetGlobalApp()->m_NfcPara.MerchantCode,MerchantBin,15);
					break;
				}
				//ACK
				if(0x06 == BinData[0])
...
const CString CNFCProtocol::ConstructParaQueryCmd(void)
{
	CString strCMD;
	strCMD.Format(_T("02 00 24 03 01 31 32 33 34 35 36 35 37 1C 38 39 38 33 31 30 31 37 33 39 39 30 38 37 37 1C 37 30 30 38 30 32 33 35 1C 03 01"));
	strCMD.Replace(_T(" "),_T(""));
	return strCMD;
}

const CString CNFCProtocol::ConstructSoftVerQueryCmd(void)
{
	CString strCMD;
	strCMD.Format(_T("02 00 27 03 00 31 32 33 34 35 36 35 32 1C 30 31 1C 38 39 38 33 31 30 31 37 33 39 39 30 38 37 37 1C 37 30 30 38 30 32 33 35 1C 03 1B"));
	strCMD.Replace(_T(" "),_T(""));
	return strCMD;
}
