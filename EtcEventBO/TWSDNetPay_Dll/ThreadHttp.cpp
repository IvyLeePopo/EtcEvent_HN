...
unsigned short CThreadHttp::m_changedIP=0;
...
{
	{
		...
		//GetGlobalApp()->RecordLog("before exit http",LOG_LEVEL_ERROR,TRUE);
		CScopeLock tmpIdle(&idleMutex);
		//GetGlobalApp()->RecordLog("start exit http",LOG_LEVEL_ERROR,TRUE);
		this->m_bExit = true;
	}
	...
	while (1)
	{
	    dRet =::MsgWaitForMultipleObjects(1, &this->m_hThread, FALSE, 100, QS_ALLINPUT);
...
BOOL CThreadHttp::InitInstance()
{
	...
	strDBFilePath.Format(_T("%s\\TWCancelRecordBuff.db3"), m_sTranstParam.PathLogDir);
	m_cTableDebitCancel.SetDBFilePath(strDBFilePath);
	//设置撤单缓存时间
	m_cTableDebitCancel.SetLastDelayTime(::GetGlobalApp()->m_iLastDelayTime);
	return TRUE;
}
...
void CThreadHttp::CheckUnProcessDebitCancelRecord()
{
	...
	try
	{
		...
		//发送撤单请求
		PostThreadMessage(WM_THREAD_HTTP_SEND_DATA_REQUEST, (WPARAM)pDVJsonDebitCancle, (LPARAM)MAKELONG(JSON_DT_SEND_DIBIT_CANCEL, MAKEWORD(0x00, 0x01)));
		//记录日志
		GetGlobalApp()->RecordLog(GetGlobalApp()->FmtStr(_T("[支付通讯线程]:检索到未递交的撤单数据，自动调用撤单操作，交易标识:%s,该撤单记录产生时间:%s"), pDVJsonDebitCancle->PayIdentifier, cDebitCancle.CreateTime.Format(_T("%Y-%m-%d %H:%M:%S"))));
	}
	catch (int& iErrorCode)
	{
		...
		{
		case -1:
			strError.Format(_T("[支付通讯线程]:检索未递交的撤单数据出现异常，无法获取到有效数据"));
			break;
		case -2:
			strError.Format(_T("[支付通讯线程]:检索未递交的撤单数据出现异常，当前正在处理扣费业务"));
			break;
		case -3:
			strError.Format(_T("[支付通讯线程]:检索未递交的撤单数据出现异常，内部申请资源失败"));
			break;
		}
		//记录日志
		if (m_strLastErrorDesc != strError)
		{
			GetGlobalApp()->RecordLog(strError);
...
void CThreadHttp::fnReadSpecailConfig()
{
	...
	//如果IP不通，则读取ipsecond的配置项-----2021-12-02
	if(m_changedIP%2 == 0)
	{
	...
	{
		::GetPrivateProfileString(_T("ReflectWeb"), _T("IPSECOND"), _T("60.205.140.198"), szIP, _countof(szIP), strCommConfigFile);
	}
	...
	m_m_dwHeartCheckMin			= (DWORD)::GetPrivateProfileInt(_T("Heart"), _T("CheckMin"), 30, m_strPathLocalConfigFile);
	m_m_dwHeartCheckMax			= (DWORD)::GetPrivateProfileInt(_T("Heart"), _T("CheckMax"), 180, m_strPathLocalConfigFile);
...
BOOL CThreadHttp::OnIdle(LONG lCount)
{
	...
	{
		//GetGlobalApp()->RecordLog("heart check",LOG_LEVEL_ERROR,TRUE);
	...
	{
		//GetGlobalApp()->RecordLog("pay cancle",LOG_LEVEL_ERROR,TRUE);
...
void CThreadHttp::OnMsgSendQueryToReflectWeb(WPARAM wParam, LPARAM lParam)
{
	...
	if ( 0 == theApp.m_iHTTPVersion )
	{
		try
		{
			...
			{
				//返回代码不在以下列表（成功/输入密码/二维码超时/二维码非法/余额不足/密码错误/状态未知），则认为是内部参数异常
				if (   (HTTP_RC_DEBIT_SUCCESS				!= pDVJsonBase->ServerResponseCode)
				        && (HTTP_RC_DEBIT_NEED_PASSWORD			!= pDVJsonBase->ServerResponseCode)
				        && (HTTP_RC_DEBIT_CODE_TIMEOUT			!= pDVJsonBase->ServerResponseCode)
				        && (HTTP_RC_DEBIT_SRC_ILLGAL				!= pDVJsonBase->ServerResponseCode)
						...
						&& (HTTP_RC_PAY_STATUS_UNKNOW          != pDVJsonBase->ServerResponseCode)
				   )
				{
					//抛出异常
					throw - 5;
				   ...
				   )
				{
					   ...
					   )
					{
						...
						m_bLoopBreak = TRUE;
					         ...
					         ||	(HTTP_RC_DEBIT_QUERY_ERROR		== pDVJsonBase->ServerResponseCode
							 || HTTP_RC_PAY_STATUS_UNKNOW  == pDVJsonBase->ServerResponseCode)//查询失败继续循环
					        )
					{
...
void CThreadHttp::OnMsgSendHeart(WPARAM wParam, LPARAM lParam)
{
	...
	{
		//更新日志
		GetGlobalApp()->RecordLog(GetGlobalApp()->FmtStr(_T("[支付通讯线程]:支付平台心跳验证,第 %d 次"), i + 1), LOG_LEVEL_RUN);
		CheckHeartState();
		Sleep(iSleep);
...
UINT WINAPI CThreadHttp::SendData(LPVOID lpParam)
{
	...
	try
	{
		...
		*/
		...
		int debug_send_data_len = pParam->SendData.GetLength();
		char debug_log[100]={0};
		sprintf(debug_log,"send length:%d",debug_send_data_len);
		GetGlobalApp()->RecordLog(debug_log);
		if( debug_send_data_len > 4000)
		{
			GetGlobalApp()->RecordLog(pParam->SendData);
		}
...
END:
	...
	{
		// static CString strLastError = _T(""); modify 2018-10 zhuyabing，将此部分内容放在前面
...
void CThreadHttp::CheckHeartState()
{
	...
	if (GetGlobalApp()->m_bNetErrorHttpServerInTollNetNotExist || GetGlobalApp()->m_bNetErrorHttpServerTrawePlatformNotExist )
	{
		...
		if ( (time(NULL) - tLastCheckHeartLogTime) > 30)
		{
			...
			m_changedIP++;//当网络异常时，读取ipsecond的配置项
...
const CString CThreadHttp::GetServerResponseCodeDesc(IN const int& iResponseType)
{
	...
	case HTTP_RC_NO_SERVICES:
		...
		break;
	case HTTP_RC_PAY_STATUS_UNKNOW:
		strReturn.Format(_T("平台状态未知"));
		break;
	default:
