...
bool PackSimilarlyContent_Upgrade(Json::Value& rootV, const LaneCodePara& lcp)
{
	...
	rootV["startGui"] = Json::Value("");
	rootV["bannerGui"]= Json::Value("");
	rootV["voiceGui"]=Json::Value("");
	//parse led version
	{
		Json::Reader jsonReader;
		Json::Value rootN;
		if(jsonReader.parse(lcp.ledVersion,rootN))
		{
			std::string tmp_read_val;

			ReadJsonStr(rootN,"Welcome",tmp_read_val);
			rootV["startGui"] =  Json::Value(tmp_read_val);
			ReadJsonStr(rootN,"Banner",tmp_read_val);
			rootV["bannerGui"] =  Json::Value(tmp_read_val);
			ReadJsonStr(rootN,"Voice",tmp_read_val);
			rootV["voiceGui"] =  Json::Value(tmp_read_val);
		}
	}
