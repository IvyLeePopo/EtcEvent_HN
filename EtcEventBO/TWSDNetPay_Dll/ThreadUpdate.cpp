...
CThreadUpdate::CThreadUpdate()
{
	...
	m_nHeartSpan = 60000;	
	m_nUpdateSpan = 300000;
	m_nDownLoadSpan = 30000;

	m_dwHeartCheck = ::GetTickCount();
	m_dwUpdateCheck = ::GetTickCount();
	m_dwDwonLoadCheck = ::GetTickCount();
...
void CThreadUpdate::ReadWebConfig()
{
	...
	//add 2017-12
	int remotePort = ::GetPrivateProfileInt(_T("ReflectWeb"),	_T("UpdatePort"),		8090,		m_csCfgFile );

	m_RAC.RemoteInfo(szbuff,remotePort);

	//add 2018-01
	m_LogUnit.SetUploadServer(szbuff,remotePort);
	

	m_nWebPort	=	::GetPrivateProfileInt(_T("IMTS"),	_T("Port"),		8080,		m_csCfgFile );

	m_nHeartSpan		=	::GetPrivateProfileInt(_T("IMTS"),	_T("HeartSpan"),		60000,		m_csCfgFile );
	m_nUpdateSpan		=	::GetPrivateProfileInt(_T("IMTS"),	_T("UpdateSpan"),		300000,		m_csCfgFile );
	m_nDownLoadSpan	=	::GetPrivateProfileInt(_T("IMTS"),	_T("DownLoadSpan"),		30000,		m_csCfgFile );
...
//#endif
	...
	/*
	if( m_nHeartSpan < 30000)//心跳间隔不能小于 30s
	{
		m_nHeartSpan = 30000;
	}
	if( m_nUpdateSpan < 180000)//更新检测间隔不能小于 180s
	{
		m_nUpdateSpan = 180000;
	}
	if( m_nDownLoadSpan < 30000 )//更新下载间隔不能小于 30s
	{
		m_nDownLoadSpan = 30000;
	}
	*/
...
void CThreadUpdate::HTTPDeal(LPVOID lpParam)
{
	...
	catch(CInternetException* e)
	{
		...
		strTemp.TrimLeft();
		strTemp.TrimRight();
		strErrorDesc.Format(_T("[播报更新线程]: HTTP处理失败,服务器异常,详细描述:(%s,任务号 %d,错误代码:%d)"), strTemp, iTaskNo, dwErorCode );
		GetGlobalApp()->RecordLog( strErrorDesc );
		e->Delete();
	}
	catch(...)
...
void CThreadUpdate::MainHeart()
{
	...
	if( dwCurrent >= m_dwHeartCheck )//正常计时，未到49.7天的溢出周期
	{
		if( dwCurrent - m_dwHeartCheck >= m_nHeartSpan)//默认 60 S检测一次
		{
	...
	{
		m_dwHeartCheck = dwCurrent;	//重置上次时间片，等待下次循环处理
	}
}
...
void CThreadUpdate::ReportHeartState()
{
	...
	try
	{
		...
		{
			strError.Format( _T("[播报更新线程]: 构造HTTP心跳json串(%s)异常: 长度过短"), strSendData );
		}
		else
		{
	...
	{
		strError.Format( _T("[播报更新线程]: 发送HTTP心跳未知异常: %d"), ::GetLastError() );
	}

	if(0 != m_strLastErrorDesc.CompareNoCase(strError))
	{
		m_strLastErrorDesc = strError;
...
CString CThreadUpdate::GetDeviceNo(int nProvince, int nRoad, CString strSTID, int nLane)
{
	...
	csNo.Format( _T("%s"),theApp.m_strTWProductSerialNo);
	return csNo;
}
...
void CThreadUpdate::MainUpdate()
{
	CString strError;
	DWORD	dwCurrent = ::GetTickCount();
	if( dwCurrent >= m_dwUpdateCheck )//正常计时，未到49.7天的溢出周期
	{
		if( dwCurrent - m_dwUpdateCheck >= m_nUpdateSpan)//默认 300 S检测一次
		{
	...
	{
		m_dwUpdateCheck = dwCurrent;	//重置上次时间片，等待下次循环处理
	}
}
...
void CThreadUpdate::CheckHTTPUpdate()
{
	...
	try
	{
		...
		if( strSendData.GetLength() < 10 )
		{
			strError.Format( _T("[播报更新线程]: 构造HTTP查询json串(%s)异常: 长度过短"), strSendData );
		}
		else
		{
			sParam.SendData	= StringConvertByCodePage(strSendData, CP_ACP, CP_UTF8);
	...
	{
		strError.Format( _T("[播报更新线程]: 发送HTTP更新检测未知异常: %d"), ::GetLastError() );
	}
...
{
	DWORD	dwCurrent = ::GetTickCount();
	if( dwCurrent >= m_dwDwonLoadCheck )//正常计时，未到49.7天的溢出周期
	{
		if( dwCurrent - m_dwDwonLoadCheck >= m_nDownLoadSpan)//默认 30 S检测一次
		{
	...
	{
		m_dwDwonLoadCheck = dwCurrent;	//重置上次时间片，等待下次循环处理
	}
}
...
int CThreadUpdate::CreateLocalDB3(LPCTSTR strDBFile, LPCTSTR strTableName, CString& csError)
{
	...
	if((err = _access_s( strDBFile, 0 )) == 0 )
	{
		...
		rc = sqlite3_agent::sqlite3_open_fun( strDBFile, &sq3db);	//SQLITE_OK
		if(rc != SQLITE_OK)
		{
			csError.Format( _T("Can't open DB(%s) database: %s\n"), strDBFile, sqlite3_agent::sqlite3_errmsg_fun(sq3db) );
			sqlite3_agent::sqlite3_close_fun(sq3db);
			return -11;
		}
		else
		{
			csSQL.Format( _T("select * from sqlite_master where name='%s';"), strTableName );
			rc = sqlite3_agent::sqlite3_prepare_fun(sq3db, csSQL.GetBuffer(0), csSQL.GetLength(), &stmt, &tail);	//SQLITE_OK
			csSQL.ReleaseBuffer();
			if(rc != SQLITE_OK)
			{
				csError.Format( _T("check DB(%s) table(%s) prepare error: %s\n"), strDBFile, strTableName, sqlite3_agent::sqlite3_errmsg_fun(sq3db) );
				sqlite3_agent::sqlite3_close_fun(sq3db);
				return -12;
			}
			else
			{
				...
				sqlite3_agent::sqlite3_finalize_fun(stmt);	//提前这里结束，是为了后面可能的建表语句
				...
				else if( rc == SQLITE_DONE)//检索成功，没有数据
				{
					...
					rc = sqlite3_agent::sqlite3_exec_fun(sq3db, csCreateTB.GetBuffer(0), NULL, NULL, &zErr);
				...
				sqlite3_agent::sqlite3_close_fun(sq3db);		//SQLITE_OK	SQLITE_ERROR	SQLITE_BUSY
	...
	{
		rc = sqlite3_agent::sqlite3_open_fun( strDBFile, &sq3db);
		if( rc != SQLITE_OK )
		{
...
int CThreadUpdate::SaveData2LocalDB3(LPCTSTR strDBFile, LPCTSTR strTBName, CUpdateData& cUpdate, CString& csError)
{
	...
	for(int i=0; i<cUpdate.m_aryUpdateInfo.GetSize(); ++i)
	{
		...
		rc = sqlite3_agent::sqlite3_exec_fun(db, csInsert.GetBuffer(0), NULL, NULL, &zErr);
		...
		if(rc != SQLITE_OK)
		{
			...
			{
				rc = sqlite3_agent::sqlite3_exec_fun(db, csCreate.GetBuffer(0), NULL, NULL, &zErr);
...
{
    std::string thisPath = fs_thisModulePath();
    std::string sd =  fs_PathJoin(thisPath, "TWSDNetPayCore.dll");
	std::string ver_info = std::string( theApp.GetVersionOfFile(sd.c_str()));


    if (ver_info.find('2') == 0)
    {
        return "TR200-V2.0";
    }
    else
    {
        return "TR200-V2.0-M1";
    }
    return "devType";

}
void CThreadUpdate::RemoteControl()
{
	static int init_RAC = m_RAC.SetLaneCodeInfo(m_sTranstParam.ProvinceID,(m_sTranstParam.LaneConfigInfo.str_RoadID),m_sTranstParam.LaneConfigInfo.str_StationID,m_sTranstParam.LaneConfigInfo.str_LaneID,m_sTranstParam.PathCommDir);
	static int init_LU = m_LogUnit.SetLaneCodeInfo(m_sTranstParam.ProvinceID,(m_sTranstParam.LaneConfigInfo.str_RoadID),m_sTranstParam.LaneConfigInfo.str_StationID,m_sTranstParam.LaneConfigInfo.str_LaneID,GetDeviceType());

	/* for debug
	if(access("test_terminal_update",0)==0)
	{
		remove("test_terminal_update");
		std::string test_info = "{\"FirmwareInfo\":{\"FirmwarePath\":\"terminal_test.bin\",\"FirmwareType\":\"FW001\",\"fileId\":\"942f75459469498ca91e49dcf074d993\",\"infoId\":\"RFI2019030615543034369837\",\"issueId\":\"2733183c4f924430b1b3c2d84ff39a05\"},\"Version\":\"1.0\"}";
		char* p_str = new char[2048];
		memset(p_str,0,2048);
		strcpy(p_str,test_info.c_str());
		::PostThreadMessage(this->m_sTranstParam.CallerThreadID,WM_THREAD_REMOTE_CONTROL_FIRMWARE,(WPARAM)p_str,0);
	...
	*/
	...
	if( (curr_time-last_control_time) < 60*5 ) //-----20211130 测试修改为1分钟的频率。 60*5
	{
		return ;
	}
	last_control_time = curr_time;
	m_RAC.UpgradeFirmware();
	std::list<std::string> firmParaList = m_RAC.TakeUpgradePara();
	for(std::list<std::string>::iterator it = firmParaList.begin();it!=firmParaList.end();++it)
	{
		char * tmp_str =new char[it->size()+1];
		if(tmp_str==NULL)
		{
			return;
		}
		memset(tmp_str,0,it->size()+1);
		strcpy(tmp_str,it->c_str());
		
		::PostThreadMessage(this->m_sTranstParam.CallerThreadID,WM_THREAD_REMOTE_CONTROL_FIRMWARE,(WPARAM)tmp_str,0);
	}

	//add 2018-01 新增日志提取
	std::string tmp_para(LPCSTR(theApp.m_strTWProductSerialNo));
	m_LogUnit.UploadLog(tmp_para);

	
}


void CThreadUpdate::RemoteUpgradeResult(WPARAM wParam)
{
	m_RAC.TellUpgradeResult(wParam);
}
