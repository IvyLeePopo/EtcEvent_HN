...
CAgencyPay::CAgencyPay(void)
{
	...
	fnIF_QueryVirtualCardDebit = NULL;
	m_bInitDll = false;//add 2017-12
...
const BOOL CAgencyPay::Init(IN const CString& strDllFilePath)
{
	if(NULL != (m_hModule = ::LoadLibrary(strDllFilePath)))
	{
		...
		fnIF_QueryVirtualCardDebit = (defIF_QueryVirtualCardDebit)::GetProcAddress(m_hModule,"IF_QueryVirtualCardDebit");
		...
		if(		(NULL != fnIF_InitEnvironment)
			 ...
			 //&& (NULL != fnIF_QueryVirtualCardDebit)
			 )
		{
			m_bInitDll = TRUE;
		}
...
{
	//CScopeLock tmpIdle(&m_accessMutex);
	if(m_bInitDll)
	{
...
int CAgencyPay::AgencyQueryVirtualCardDebit(IN const char* szParamContext, IN const int& iParamSize, IN const int& iFormatType, OUT char* szResult)
{
	if( m_bInitDll &&fnIF_QueryVirtualCardDebit)
	{
		return fnIF_QueryVirtualCardDebit(szParamContext,iParamSize,iFormatType,szResult);
	}
	return 255;
}