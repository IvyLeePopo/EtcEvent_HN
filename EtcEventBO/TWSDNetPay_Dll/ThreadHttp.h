...
public://add 2017-11 原有的代码在线程退出的时候存在众多问题
	void waitTillIdle();
private://add 2017-11
	CRITICAL_SECTION idleMutex;
	bool goDie;
	int dieNumber;//2017 有效
	bool isAlive;
	bool isFirstRun;
protected:
...
protected:
	...
	//需要发送一次心跳到检测状态
	afx_msg void OnMsgSendHeart(WPARAM wParam,LPARAM lParam);
...
protected:
	...
	static UINT  WINAPI SendData_ByWinNet(LPVOID lpParam);
	//数据格式转换,从调用者传过来的DVJson格式转换成字符串形式
	const BOOL DataConvertFromJson2String(IN const WORD& wSendDataType,IN OUT CDVJsonBase* pDVJsonReceive,IN OUT CString& strURL,IN OUT INTERNET_PORT& wPort,IN OUT CString& strSendData,IN OUT HWND& hWnd,IN OUT int& iCallMode,IN OUT CJsonOperationIF** pJsonIF,int& iPayChannelType, int nSN=0 );
	//测试指定IP计算机是否在同一个网络内
...
private:
	...
	//心跳检测时间间隔最小阈值
	DWORD	m_m_dwHeartCheckMin;
	//心跳检测时间间隔最大阈值
	DWORD	m_m_dwHeartCheckMax;
	...
	//增加ipsecond配置项之后，如果IP不生效，则读取ipsecond的配置 --- 2021-12-02
	static unsigned short m_changedIP;


public:
	BOOL	m_bLoopBreak;	//扣费循环的中断标记
};
