...
//ETC特情处理专用回调函数
typedef void ( *NotifyCallerEtcEvent)(int iOption, int iResult, void* pIndPtr);
//typedef void (*NotifyCallerFun)(int iOption, int iResult, void* pIndPtr);
...
class CTWSDNetPay_DllApp : public CWinApp ,public trawe::CMessageTerminal
{
public:
	CTWSDNetPay_DllApp();
...
public:
	...
	static bool WINAPI  IF_QueryVirtualCardDebit(IN const char* szParamContext, IN const int& iParamSize, IN const int& iFormatType, IN int& iSupportAgent, OUT char* szResult);
	...
	static int ETCInterfaceBasicCheck(const char* szParamContext, const int& iParamSize );
	static int WINAPI IF_QueryAgentDebitEtcUp(IN const char* szParamContext, IN const int& iParamSize, IN const int& iFormatType, IN int& iSupportAgent, IN const int& iTimeout);
	static int WINAPI IF_QueryAgentDebitResultEtcUp(IN const char* szParamContext, IN const int& iParamSize, IN const int& iFormatType, IN OUT char* szResultContext, IN OUT int& iResultSize, IN int iTimeout);
	static int WINAPI IF_DebitMoneyEtcUp(IN const char* szParamContext, IN const int& iParamSize, IN const int& iFormatType, IN OUT char* szResultContext, IN OUT int& iResultSize, IN int iTimeout);
	
	int QueryImageIdFromLPRSystem(std::string& image_id,std::string& task_id);
	static bool WINAPI  IF_DebitMoney_Future(IN const char* szParamContext, IN const int& iParamSize, IN const int& iFormatType);
	static bool WINAPI  IF_DebitCancel_Future(IN const char* szParamContext, IN const int& iParamSize, IN const int& iFormatType);
	static bool WINAPI  IF_GetDebitResult_Future(IN OUT char* szParamContext, IN OUT int& iParamSize, IN const int& iFormatType);

	static bool WINAPI  IF_LockDll();//锁定dll，只允许调用销毁接口

	/*****************************ETC特情专用接口**********************************************************/
	//初始化组件环境
	static bool WINAPI  IF_EventInitEnvironment3(IN const UINT& nThreadID, IN const HWND& hWnd, IN const unsigned int& nNotifyMsgID, IN const char* szAreaInfo, 
		IN const char* szLoaclStation, IN const char* szLoaclLaneID, IN const char* szServerInfo, 
		IN const int& iProvinceID, IN OUT void* pIndPtr,void (*fun)(int option, int result,void* pIndPtr));
	//清理资源
	static bool WINAPI  IF_EventDestroy(void);
	//获取上一次错误信息
	static const char* WINAPI IF_EventGetLastErrorDesc(void);
	//获取组件状态
	static bool WINAPI IF_EventGetComponentStatus(IN OUT unsigned int& uStatus);
	//特情处理开始
	static bool WINAPI IF_EventDealStart(IN const char* szParamContext, IN const int& iParamSize, IN const int& iFormatType);
	//车辆队列确认
	static bool WINAPI IF_EventCheckVehQueue(IN const char* szParamContext, IN const int& iParamSize, IN const int& iFormatType);
	//车辆信息确认
	static bool WINAPI IF_EventCheckVehInfo(IN const char* szParamContext,IN const int& iParamSize,IN const int& iFormatType);
	//入口信息确认
	static bool WINAPI IF_EventCheckEntryInfo(IN const char* szParamContext,IN const int& iParamSize,IN const int& iFormatType);
	//费额显示
	static bool WINAPI IF_EventShowFeeInfo(IN const char* szParamContext,IN const int& iParamSize,IN const int& iFormatType);
	//支付状态显示
	static bool WINAPI IF_EventPayResultDisplay(IN const char* szParamContext,IN const int& iParamSize,IN const int& iFormatType);
	//特情处理结束
	static bool WINAPI IF_EventDealStop(IN const char* szParamContext,IN const int& iParamSize,IN const int& iFormatType);
	//启动扫码
	static bool WINAPI IF_EventStartScan(IN const int& iFeeValue);
	//停止扫码
	static bool WINAPI IF_EventStopScan();
	//ETC卡操作提示
	static bool WINAPI IF_EventCardOperationNotify(IN const char* szParamContext, IN const int& iParamSize, IN const int& iFormatType);
	//车辆队列修改
	static bool WINAPI IF_EventModifyVehQueue(IN const char* szParamContext, IN const int& iParamSize, IN const int& iFormatType);
	//最小费额授权
	static bool WINAPI IF_EventFeeAuthorize(IN const char* szParamContext, IN const int& iParamSize, IN const int& iFormatType);
	//自由语音播报
	static bool WINAPI IF_EventFreeVoicePlay(IN const char* szParamContext,IN const int& iParamSize,IN const int& iFormatType);
	// 其他授权接口（最小费额除外）
	static bool WINAPI IF_EventAuthorize(IN const char* szParamContext,	IN const int& iParamSize,IN const int& iFormatType);
	//车辆队列删除结果
	static bool WINAPI IF_EventDelVehQueueResult(IN const char* szParamContext,	IN const int& iParamSize,IN const int& iFormatType);
	//传输现金发票数据
	static bool WINAPI  IF_TransferCashInvoiceData(IN const char* szInvoiceData, IN const int& iSize, IN const int& iFormatType);
	//撤销现金发票数据
	static bool WINAPI  IF_CancelCashInvoiceData(IN const char* szInvoiceData, IN const int& iSize, IN const int& iFormatType);
	
	static bool CheckEventDealStartJsonValidity(const char* szParamContext, CString& ErrorDescribe);
	static bool CheckEventCheckVehQueueJsonValidity(const char* szParamContext, CString& ErrorDescribe);
	static bool CheckEventShowFeeInfoJsonValidity(const char* szParamContext, CString& ErrorDescribe);
	static bool CheckEventPayResultShowJsonValidity(const char* szParamContext, CString& ErrorDescribe);
	static bool CheckEventDealStopJsonValidity(const char* szParamContext, CString& ErrorDescribe);
	static bool CheckEventCheckVehInfoJsonValidity(const char* szParamContext, CString& ErrorDescribe);
    static bool CheckEventCheckEntryInfoJsonValidity(const char* szParamContext, CString& ErrorDescribe);
	static bool CheckCardOperationNotifyJsonValidity(const char* szParamContext, CString& ErrorDescribe, int& NotifyType);
	static bool CheckEventModifyVehQueueJsonValidity(const char* szParamContext, CString& ErrorDescribe);
	static bool CheckAuthorize(const char* szParamContext, CString& ErrorDescribe);
...
public:
	int CreateTasks();
    virtual int ExecuteMessage(int src_key,char* msg,int msg_len);
	void DataForwardRead(int destKey, char *message, int len, int srcKey);
    void DataForwardWrite(int destKey, char *message, int len,  bool pointerFlag=false);
	void StopListenMessage();
	CRITICAL_SECTION m_DllAccessMutex;
private:
	void ConnectDataForward();
    void DisonnectDataForward();

	void DataForwardRegister( const std::string& topic);
    void DataForwardUnregister();
	const int m_SendId;
    HMODULE  m_Forward_Dll;

    DataForward_Register m_Forward_RegisterFun;
    DataForward_Unregister m_Forward_UnregisterFun;
    DataForward_Write m_Forward_WriteFun;
    DataForward_Read m_Forward_ReadFun;
	trawe::CComponentMessage* m_MessageCenter;
private:
	...
	bool IsProvinceTianJin();
	void SetLPRQueryOverTimeMilli(int LPRQueryOverTimeMilli=100);
	int GetLPRQueryOverTimeMilli();
	//add 2018-09
	void ResetDebitResult();
	void GetETCDataFromAgencyResult(const char* result);
...
public:
	...
	trawe::ETCPayManage m_EtcPayCenter;


	//动态库加载句柄
	HMODULE	m_hModule_Device;
...
public:
	...
	int NotifyPayStatus(int type,int value);
private:/*实现组件平台接口*/
	void ConnectBus();
	void DisConnectBus();
...
#else
	static void WINAPI RecordLog(IN const CString &strLog, IN const int& iLevel = 1, IN const BOOL& bForce = TRUE);
#endif
	//线程关闭函数
...
public:
	...
	//解析代扣扣款接口返回来的数据
	static int GetAgencyResult(CString strXML, DWORD dwConsumeTime,int&debitType);//modify 2018-09 保存原始类型
	//构造代扣扣费数据并执行代扣
	static bool AgencyConstructDebitData(int & agency_result);
	//构造代扣撤单数据并执行撤单
...
public:
	...
	//存储EtcEvent上次错误信息指针
	static CString m_strEtcEventLastErrorDesc;
	//存储虚拟卡号前10位
	...
	//标识组件通知外部使用的回调函数指针(ETC特情处理专用)
	static NotifyCallerEtcEvent  m_CallbackNotifyEtcEvent;
	//ETC特情处理应答指针，在回调通知的时候将应答内容传递出去
	static void* m_pIndPtrEtcEvent;

	//标识EtcEvent环境是否已正常初始化
	static bool m_bIsEtcEventEnvironmeInited;

	//通知回调函数调用线程
	static CThreadNotifyCaller* m_pThreadNotifyCaller;
	//回调函数通知线程的线程id
	static UINT  NotifyCallerThreadID;
	//设备管理线程的线程id
	static UINT  DevMangerThreadID;

	//EtcEvent应答消息ID（河南要求通过消息通知的方式进行EtcEvent应答）
	static UINT  EtcEventRespMessgeID;
	//EtcEvent调用者窗体句柄
	static HWND	 EtcEventCallerWnd;
	//EtcEvent调用者线程ID
	static UINT	 EtcEventCallerThreadID;
	static BOOL  m_bEnableMediaGateWay;
	static UINT  m_iCurrentWorkMode;  //0：ETC，1：MTC

	//EtcEvent A5协议内容相关字段
	int m_EtcProvinceId;
	std::string m_EtcProvinceName;
	std::string m_EtcAreaId;
	std::string m_EtcAreaName;
	std::string m_EtcRoadId;
	std::string m_EtcRoadName;
	std::string m_EtcStationId;
	std::string m_EtcStationName;
	std::string m_EtcLaneId;		
	std::string m_EtcServerIP;
	int m_EtcServerPort;
