...
#define TW_TRACE_DIARY_LOGUP(x)  WriteLog(x)
...
void LogUploader::AskCmdId()
{
	...
	m_HttpLink.ChangeRemotePeer((changeIP%2)?GetSecondPayIP():GetPayIP(), GetUpdatePort());
	char sLog[1024] = {0};
	sprintf_s(sLog,"日志上传心跳发送请求地址:  http://%s:%d/trawe-itms/interface/t_heartbeat",m_HttpLink.m_IpStr.c_str(),m_HttpLink.m_Port);
	TW_TRACE_DIARY_LOGUP( std::string(sLog));
	...
	//------如果服务器返回的数据不能正常解析，也认为是网络异常，需要切换IPsecond
	Json::Reader  jsonReader;
	Json::Value rootN;
	if (!jsonReader.parse(askResponsePackage.c_str(), rootN))
	{
		changeIP++;
	}
...
void LogUploader::ParseTargetCmdId(const std::string cmdId)
{
    ...
    {
		TW_TRACE_DIARY_LOGUP("/trawe-itms/interface/t_cmd_query ResponsePackage:"+ResponsePackage);
...
void LogUploader::UploadFile(const LogCmdPair lcp)
{
    ...
    {
		TW_TRACE_DIARY_LOGUP("unkown cmdType:"+lcp.second.cmdType);
        return ;
    }
	...
	if(zipPath.empty())
	{
		TW_TRACE_DIARY_LOGUP("nothing to upload,notify success");
		std::string ResponsePackage;
    ...
    {
		TW_TRACE_DIARY_LOGUP("open file failed:"+zipPath);
        return;
    }
