...
void RemoteAccessCore::Query()
{
	...
	char sLog[1024] = {0};
	sprintf_s(sLog,"发送请求地址:  http://%s:%d%s",m_HttpLink.m_IpStr.c_str(),m_HttpLink.m_Port,m_queryPath.c_str());
	WriteLog( std::string(sLog));
	...
	//m_LCP.deviceFlag =  "{\"masterDeviceFlag\":\"181225_r1627\",\"slaveDeviceFlag\":\"Ver_2.1.2_20171216R01\"}";
	m_LCP.softFlag = GetSoftFlag();
	CString strVal;
	if(theApp.m_pThreadManage->m_pThreadDevice->QueryLEDVersion(strVal))
	{
		m_LCP.ledVersion = std::string(LPCSTR(strVal));
	}
	...
	////当返回的数据不能正常解析，服务器异常，也需要切换到ipsecond ----- 2021-12-03
	Json::Reader  jsonReader;
	Json::Value rootN;
	if (!jsonReader.parse(queryRespnosePackage.c_str(), rootN))
	{
		changeIP++;
	}
