...
void CThreadManage::MoveConfigFileToExeDir()
{
	...
	std::string updateDir = fs_PathJoin(fs_thisModulePath(),"TWUpdate"); //禁止删除该文件夹，更新库还在其中
...
//业务库更新之后，由于框架库和更新库都无法进行更新，加载的更新库和框架还是原来的，只能在业务库中处理一次----20211126 lhq
void CThreadManage::modifyUpdateflagResult()
{
	std::string flagFilePath = fs_PathJoin(fs_thisModulePath(),"updateflag.txt"); //禁止删除该文件夹，更新库还在其中
	...
	TW_TRACE_DIARY_TYPE("CThreadManage::modifyUpdateflagResult()! 更改updateflag.txt文件的result为：3", 12);
...
BOOL CThreadManage::InitInstance()
{
	...
	////如果需要设备库一起更新，需要重启系统，让现场的框架库不能还原设备库，并且需要在新的业务库起来时，马上加载设备库， ----- 2021-12-05 
	////if (m_bReBootSystem)
	////{
	////	RestartWindows();
...
void CThreadManage::OnMsgDebitCancel(WPARAM wParam, LPARAM lParam)
{
	...
	try
	{
		...
		m_pThreadHttp->m_bLoopBreak = TRUE;	//需要跳出扣费执行循环
...
void CThreadManage::OnMsgCheckDebitPasswordState(WPARAM wParam, LPARAM lParam)
{
	if(!GetGlobalApp()->m_bIsTrading)
	{
		GetGlobalApp()->RecordLog(_T("未处于交易中，停止查询"));
		return ;
	}
...
void CThreadManage::fnReleaseResource()
{
	...
	try
	{
		...
		::Sleep(100);
		GetGlobalApp()->RecordLog("播报更新线程已经退出", LOG_LEVEL_RUN, TRUE);
		...
		::Sleep(dwSleepTime);
		GetGlobalApp()->RecordLog("设备控制线程已经退出", LOG_LEVEL_RUN, TRUE);
		...
		GetGlobalApp()->RecordLog("支付通讯线程已经退出", LOG_LEVEL_RUN, TRUE);
		...
		GetGlobalApp()->RecordLog("NFC签到线程已经退出", LOG_LEVEL_RUN, TRUE);
	}
	catch (int& iErrorCode)
	{
	}
...
BOOL CThreadManage::ContructDebitMoneyInfo(IN const tagAccountInfo* pAccountInfo)
{
	...
	if(IsFuturePay())
	{
		return FALSE;//remove 2019-01 使用新支付
	}
...
// 	return true;
	...
	catch (int& iErrorCode)
	{
		...
		case -1:
			break;
		case -2:
			theApp.RecordLog("ThreadDevice线程启动失败！");
			break;
		case -3:
			break;
		}
		...
		else
		{
			...
			if (NULL != ((CThreadManage*)dwUser)->m_pDVJsonDebitCheck )
			{
				...
				if (NULL != pDVJsonDebitCheck)
				{
					...
					//记录日志
					//add 2018-01 ,增加了对总超时时间的日志输出。
					GetGlobalApp()->RecordLog(GetGlobalApp()->FmtStr(_T("[业务管理线程]:扣款操作超时未截至（已累加 %d 毫秒/%d），根据规则，自动进行下一次的扣款状态检测，交易标识:%s"), ((CThreadManage*)dwUser)->m_dwCheckDebitAccumulateTime,((CThreadManage*)dwUser)->m_dwInputPasswordDelayTime, pDVJsonDebitCheck->PayIdentifier));
...
//				StartTimer(TIMER_ID_NFC_WAIT_CORRECT_DATA,GetGlobalApp()->m_iNFCGetCancelDataLimitTime * 1000);
	...
	catch (int& iErrorCode)
	{
		...
		case -2:
			break;
		case -3:
			strLog.Format(_T("[业务管理线程]:NFC非接数据处理，由于当前不在交易流程当中，该数据不予以处理"));
			break;
		}
	}
	//记录日志
...
{
	//add zhuyabing 2017-11 修复NFC超时逻辑
	if (::GetGlobalApp()->m_bIsTrading)
	{
		/*
...
{
	CString strLog;
	strLog.Format(_T("[业务管理线程]:解析现金发票数据XML并启动上传流程..."));
	GetGlobalApp()->RecordLog(strLog);
    ...
    if (!read_success)
    {
        TW_TRACE_DIARY("现金发票XML参数解析错误:" + GetXMLParseError());
        return false;
    }
...
bool CThreadManage::TransferCashInvoiceDataJSON(const char* szInvoiceData, const int& iSize)
{
	...
	if (!read_success)
    {
        TW_TRACE_DIARY("现金发票JSON参数解析错误:" + GetJsonParseError());
        return false;
    }
	//组装二维码数据
	/*std::stringstream ss;
...
{
	CString strLog;
	strLog.Format(_T("[业务管理线程]:解析【撤销】现金发票数据XML并启动撤销流程..."));
	GetGlobalApp()->RecordLog(strLog);
	...
	if (!read_success)
    {
        TW_TRACE_DIARY("撤销现金发票XML参数解析错误:" + GetXMLParseError());
        return false;
    }
...
{
	CString strLog;
	strLog.Format(_T("[业务管理线程]:解析【撤销】现金发票数据JSON并启动撤销流程..."));
	GetGlobalApp()->RecordLog(strLog);
	...
	if (!read_success)
    {
        TW_TRACE_DIARY("撤销现金发票JSON参数解析错误:" + GetJsonParseError());
        return false;
    }
	int ret = m_PayCenter.StartCashInvoiceCancel(param);
