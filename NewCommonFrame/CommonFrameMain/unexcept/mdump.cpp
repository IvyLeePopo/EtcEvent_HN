...
static bool g_IsDumpWithFullMemory = false;
...
static void MsgError( const char* sDst )
{
	...
	char errMsg[1024] = {0};
	sprintf_s(errMsg,1024,"%s (%s)",sDst,(char*)lpMsgBuf);
...
static bool GetDumpFilePath(std::string& strDumpPath)
{
    ...
    {
        sprintf_s(buf,
                  "\\%s_%4d%02d%02dT%02d%02d%02d-%d.dmp",
                  strProgName.c_str(),
                  now->tm_year+1900,
                  now->tm_mon+1,
                  now->tm_mday,
...
//    AvtWriteLog(AVT_LOG_INFO, "dump[%s]", strDumpPath.c_str());
//    sprintf_s(buf, "\\%s.dmp", strProgName.c_str());
//    strDumpPath = sDumpPath + buf;
	...
	if (hDll)
	{
		...
		if (pDump)
		{
			...
			if (hFile!=INVALID_HANDLE_VALUE) 
			{
				...
				_MINIDUMP_TYPE eDumpType = MiniDumpNormal;
